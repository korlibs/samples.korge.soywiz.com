(function (root, factory) {
  ...
  var update_0 = defineInlineFunction('kotlinx-atomicfu.kotlinx.atomicfu.update_toml5v$', function ($receiver, function_0) {
    while (true) {
      var cur = $receiver.value;
      var upd = function_0(cur);
      var compareAndSet_dqye30$result;
      compareAndSet_dqye30$break: do {
        if ($receiver.value !== cur) {
          compareAndSet_dqye30$result = false;
          break compareAndSet_dqye30$break;
        }
        $receiver.value = upd;
        compareAndSet_dqye30$result = true;
      }
       while (false);
      if (compareAndSet_dqye30$result)
        return;
    }
  });
  ...
  AtomicBoolean.prototype.compareAndSet_dqye30$ = defineInlineFunction('kotlinx-atomicfu.kotlinx.atomicfu.AtomicBoolean.compareAndSet_dqye30$', function (expect, update) {
    if (this.value !== expect)
      return false;
    this.value = update;
    return true;
  });
  AtomicBoolean.prototype.getAndSet_6taknv$ = function (value) {
    var oldValue = this.value;
    this.value = value;
    return oldValue;
  };
  ...
  package$atomicfu.atomic_6taknv$ = atomic_2;
